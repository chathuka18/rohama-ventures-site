...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-navy-900/5 bg-[#F7F9FC]/85 backdrop-blur-xl">
      <div className="container-shell flex h-20 items-center justify-between">
          ...
          <div className="relative" onMouseEnter={()=>setMega(true)} onMouseLeave={()=>setMega(false)}>
            ...
            {mega && (
              <div className="absolute left-1/2 top-[72px] w-[760px] -translate-x-1/2 rounded-3xl border border-navy-900/10 bg-white p-7 shadow-soft">
                <div className="mb-5 flex items-center justify-between"><span className="eyebrow">Our Services</span><Link className="text-sm font-medium text-navy-900" to="/services">View all services →</Link></div>
        ...
        <div className="hidden lg:block"><Link to="/contact" className="inline-flex items-center gap-2 rounded-full bg-navy-900 px-5 py-3 text-sm font-medium text-white transition hover:bg-navy-800">Let's Talk <ArrowUpRight size={15}/></Link></div>
        <button className="lg:hidden" onClick={()=>setOpen(!open)}>{open ? <X/> : <Menu/>}</button>
      </div>
      {open && <div className="border-t border-navy-900/5 bg-white p-6 lg:hidden"><div className="flex flex-col gap-4 text-base"><Link onClick={()=>setOpen(false)} to="/about">About</Link><Link onClick={()=>setOpen(false)} to="/services">Services</Link><Link onClick={()=>setOpen(false)} to="/our-approach">Our Approach</Link><Link onClick={()=>setOpen(false)} to="/clients">Clients</Link><Link onClick={()=>setOpen(false)} to="/insights">Insights</Link><Link onClick={()=>setOpen(false)} to="/contact" className="mt-2 rounded-full bg-navy-900 px-5 py-3 text-center text-white">Let's Talk</Link></div></div>}
    </header>
  )
}
